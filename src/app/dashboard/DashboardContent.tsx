...
import type { AgentStats, AgentModule, RankingItem, ChartDataPoint, AgentCapability } from '@/types';
...
const translations = {
  zh: {
    ...
    notifications: {
      ...
      agent_deployed: { title: 'Agent 已部署', message: '{name} 就绪。加入赛季即可参赛。' },
      agent_deleted: { title: '销毁完成', message: 'Agent 实例已移除。' },
      withdrawn: { title: '已退出赛季', message: '已离开赛场，历史成绩已归档。' },
      joined: { title: '成功参赛', message: 'Agent 已进入第四赛季池。' },
      prompt_saved: { title: '配置已保存', message: '[{capability}] 指令已更新。' }
    }
  },
  en: {
    app: {
    ...
    confirm_modal: {
      ...
      delete_agent_message: 'Are you sure you want to destroy this Agent instance? This action is irreversible.',
      withdraw_title: 'Withdraw from Season',
      withdraw_message: 'Are you sure? You will stop earning season points and your rank will freeze.'
    },
    notifications: {
      auth: { title: 'Authenticated', message: 'Welcome, {name}.' },
      logout: { title: 'Disconnected', message: 'User logged out securely.' },
      agent_deployed: { title: 'Agent Deployed', message: '{name} is ready. Join season to compete.' },
      agent_deleted: { title: 'Destruction Complete', message: 'Agent instance removed.' },
      withdrawn: { title: 'Withdrawn', message: 'Exited competition. Records archived.' },
      joined: { title: 'Competition Joined', message: 'Agent entered Season 4 pool.' },
      prompt_saved: { title: 'Configuration Saved', message: '[{capability}] prompt updated.' }
    }
  }
};
...
export default function DashboardContent() {
  ...
  const { 
    ...
    setAgentName, setAgentClass, setAgentStats, setAgentModules, updateCustomPrompt,
    setIsJoinedCompetition, clearAgent
  } = useAgentStore();
  const { 
    theme, activeSideTab, isChatOpen, highlightedAgent, inspectingAgent,
    selectedCapability, editingCapability,
    setTheme, setActiveSideTab, setIsChatOpen, setHighlightedAgent,
    setInspectingAgent, setSelectedCapability, setEditingCapability
  } = useUIStore();
  const { 
    isLoginModalOpen, isCreateModalOpen, isSeasonPassOpen, 
    isJoinCompetitionModalOpen, isNotifHistoryOpen, confirmModal,
    readingArticle, pendingAction,
    setIsLoginModalOpen, setIsCreateModalOpen, setIsSeasonPassOpen,
    setIsJoinCompetitionModalOpen, setIsNotifHistoryOpen, setConfirmModal,
    setReadingArticle, setPendingAction
  } = useModalStore();
  const { notifications, notificationHistory, addNotification, dismissNotification, clearHistory } = useNotificationStore();
  const { language, setLanguage } = useLanguageStore();
  const { chartData, rankingList, setChartData, setRankingList } = useMarketDataStore();
  ...
  const notify = useCallback((title: string, message: string, type: 'info' | 'success' | 'warning' | 'error' | 'market' = 'info') => {
  ...
  useEffect(() => {
    fetch('/api/rankings')
      .then(res => res.json())
      .then((data: { rankings: RankingItem[]; chartData: ChartDataPoint[] }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setRankingList(rankingList.map((agent) => {
        const change = (Math.random() - 0.48) * 0.2;
        const newProfit = agent.rawProfit + change;
        return {
          ...agent,
          rawProfit: parseFloat(newProfit.toFixed(2)),
          profit: `${newProfit >= 100 ? '+' : ''}${(newProfit - 100).toFixed(2)}%`
        };
      }).sort((a, b) => b.rawProfit - a.rawProfit).map((a, i) => ({ ...a, rank: i + 1 })));
      ...
      if (lastPoint) {
        Object.keys(lastPoint).filter(k => k !== 'time').forEach(key => {
  ...
  const handleLogin = (username: string, email?: string) => {
    ...
    notify(t.notifications.auth.title, t.notifications.auth.message.replace('{name}', username), 'success');
  ...
  const handleLogout = () => {
    clearUser();
    clearAgent();
    notify(t.notifications.logout.title, t.notifications.logout.message, 'info');
  };
  ...
  const handleCreateAgent = (
    ...
    notify(t.notifications.agent_deployed.title, t.notifications.agent_deployed.message.replace('{name}', name), 'success');
  ...
  const handleDeleteAgent = () => {
    ...
    clearAgent();
    notify(t.notifications.agent_deleted.title, t.notifications.agent_deleted.message, 'success');
  ...
  const handleJoinCompetition = () => {
    ...
    notify(t.notifications.joined.title, t.notifications.joined.message, 'success');
  };

  const handleWithdraw = () => {
    setIsJoinedCompetition(false);
    notify(t.notifications.withdrawn.title, t.notifications.withdrawn.message, 'info');
    setConfirmModal({ isOpen: false, title: '', message: '', action: () => {} });
  };
  ...
  const handleSavePrompt = (newPrompt: string) => {
    if (editingCapability) {
      ...
      notify(
        ...
        t.notifications.prompt_saved.message.replace('{capability}', editingCapability),
        'success'
      );
      ...
      <header className='shrink-0 h-[60px] bg-cp-black/80 backdrop-blur-sm border-b border-cp-border flex items-center justify-between px-6 z-50'>
        <div className='flex items-center gap-4'>
          <a href="/" className='flex items-center gap-3 group'>
            <Hexagon className='text-cp-yellow group-hover:scale-110 transition-transform' size={28} strokeWidth={1.5} />
        ...
        <aside className='w-[380px] shrink-0 border-r border-cp-border flex flex-col bg-cp-black/40 backdrop-blur-sm'>
          ...
          <div className='flex shrink-0 h-[40px] border-b border-cp-border'>
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 text-[11px] font-bold uppercase tracking-widest transition-colors ${
                activeSideTab === 'MY_AGENT' ? 'text-cp-yellow border-b-2 border-cp-yellow' : 'text-cp-text-muted'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 text-[11px] font-bold uppercase tracking-widest transition-colors ${
                activeSideTab === 'RANKING' ? 'text-cp-yellow border-b-2 border-cp-yellow' : 'text-cp-text-muted'
              }`}
            >
          ...
          <div className='flex-1 overflow-hidden min-h-0'>
            {activeSideTab === 'MY_AGENT' ? (
              ...
              <RankingList
                data={rankingList}
                onSelectAgent={(name) => setHighlightedAgent(highlightedAgent === name ? null : name)}
        ...
        <main className='flex-1 flex flex-col min-h-0 bg-cp-black overflow-hidden'>
          ...
          <div className='flex-1 min-h-0 p-4 flex flex-col'>
            ...
            <div className='flex-1 min-h-0 border border-cp-border bg-cp-dark/20 rounded-lg overflow-hidden'>
              <EquityChart
                ...
                onChartClick={(name) => setHighlightedAgent(highlightedAgent === name ? null : name)}
                theme={theme}
              />
            </div>
          </div>
          ...
          <div className='h-[280px] shrink-0 border-t border-cp-border'>
            <SeasonInfoPanel
              ...
              onOpenReport={(article) => setReadingArticle(article)}
              onSelectCapability={(cap) => setSelectedCapability(cap)}
            />
          </div>
        </main>
