...
import { MessageSquare, MinusSquare, Send, Terminal, X, Move, Trash2 } from 'lucide-react';
...
const ChatWidget: React.FC<ChatWidgetProps> = ({ isOpen, onClose }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    ...
    return [
      { role: 'model', content: '链接已建立。我是操作员。请问需要什么分析协助？', timestamp: Date.now() }
    ];
  });
  ...
  const handleClearHistory = () => {
      const initialMsg: ChatMessage = { role: 'model', content: '链接已建立。我是操作员。请问需要什么分析协助？', timestamp: Date.now() };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'model',
        content: "Error: Connection disrupted. Please try again.",
        timestamp: Date.now()
      }]);
    } finally {
      setIsThinking(false);
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-cp-yellow">
          ...
          <span className="font-bold tracking-wider font-sans text-base">操作员 // 链接</span>
        </div>
        <div className="flex items-center gap-3">
            ...
            <Move size={14} className="text-cp-text-muted group-hover:text-cp-text hidden md:block" />
            <button onClick={onClose} className="text-cp-text-muted hover:text-cp-yellow transition-colors p-1" onMouseDown={(e) => e.stopPropagation()}>
            <X size={20} />
            </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-base custom-scrollbar bg-cp-black">
        ...
        {isThinking && messages[messages.length - 1]?.role === 'user' && (
          <div className="text-cp-cyan text-sm animate-pulse pl-2 font-mono">> 正在分析输入...</div>
        )}
