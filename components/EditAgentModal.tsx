...
import { AgentStats, AgentModule } from '../types';
...
interface EditAgentModalProps {
  ...
  onSave: (name: string, agentClass: string, stats: AgentStats, modules: AgentModule[], prompt: string) => void;
  onClose: () => void;
}

const EditAgentModal: React.FC<EditAgentModalProps> = ({ initialName, initialClass, initialStats, initialPrompt, onSave, onClose }) => {
      ...
      <div className="w-full md:max-w-5xl h-[85vh] bg-cp-black border border-cp-border flex flex-col shadow-2xl relative">
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-8">
            <div className="w-full max-w-md space-y-8 text-center hover-card p-8 border border-transparent">
                 <h3 className="text-2xl font-serif font-bold text-white">Identity Update</h3>
                 <input 
                    type="text" 
                    value={name} 
                ...
                />
                 <button 
                    onClick={() => { onSave(name, initialClass, initialStats, [], initialPrompt); onClose(); }} 
