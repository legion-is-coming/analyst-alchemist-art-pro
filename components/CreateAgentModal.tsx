...
import { ChevronRight, X, Shield, Zap, Activity, Brain, Cpu, Check, ChevronLeft, Upload, FileText, Play, RotateCcw, AlertTriangle, Trash2, Clock } from 'lucide-react';
import * as echarts from 'echarts';
import { AgentStats, AgentModule, AppNotification } from '../types';

interface CreateAgentModalProps {
  onCreate: (name: string, prompt: string, archetype: string, stats: AgentStats, modules: AgentModule[]) => void;
  onClose: () => void;
...
type CreationStep = 'naming' | 'preset' | 'configure' | 'knowledge' | 'simulation';

const CreateAgentModal: React.FC<CreateAgentModalProps> = ({ onCreate, onClose, onNotify }) => {
  ...
  // Step 3: Configuration Form State
  ...
  const [simLogs, setSimLogs] = useState<string[]>(['> 系统就绪...', '> 请选择回测周期并启动。']);
  ...
  const selectedPreset = STRATEGY_PRESETS.find(p => p.id === selectedPresetId);

  // Initialize form when preset is selected
  const handlePresetSelect = (presetId: string) => {
      ...
      // Removed auto-advance: setStep('configure');
  };
  ...
  const startSimulation = () => {
       ...
       setSimLogs(['> 初始化回测环境...', '> 加载历史数据...', `> 设置回测周期: ${simDurationMode}`, '> 开始执行策略模拟...']);
       setSimResult(null);
       ...
       const interval = setInterval(() => {
           count++;
           if (Math.random() > 0.6) {
               setSimLogs(prev => [...prev, `> 模拟交易日 Day ${count}: 信号检测中...`]);
           }
       }, 200);
       ...
       setTimeout(() => {
           ...
           setSimResult({
               ...
               equityCurve: Array.from({length: 30}, (_, i) => 100 + Math.random() * 20 + i * 0.5)
           });
       }, 2500);
  };
  ...
  useEffect(() => {
    if (step === 'simulation' && simResult && chartRef.current) {
        ...
        chartInstance.current.setOption({
            ...
            xAxis: { type: 'category', data: simResult.equityCurve.map((_, i) => i), axisLine: { lineStyle: { color: 'rgba(255,255,255,0.1)' } }, splitLine: { show: false } },
            yAxis: { type: 'value', splitLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } }, axisLabel: { color: '#888' } },
            series: [{ type: 'line', data: simResult.equityCurve, smooth: true, itemStyle: { color: '#C5A059' }, areaStyle: { opacity: 0.1, color: '#C5A059' }, showSymbol: false, lineStyle: { width: 2 } }]
        });
    }
  ...
  const getStepNumber = () => {
      switch(step) {
          case 'naming': return '01';
          case 'preset': return '02';
          case 'configure': return '03';
          case 'knowledge': return '04';
          case 'simulation': return '05';
          default: return '00';
      }
  };
  ...
  const getStepTitle = () => {
      switch(step) {
          case 'naming': return '初始化身份';
          case 'preset': return '核心策略选择';
          case 'configure': return '系统参数配置';
          case 'knowledge': return '知识库接入';
          case 'simulation': return '回测模拟';
          default: return '';
      }
  };

  const isConfigValid = () => {
      return configForm.riskLevel && configForm.tradingFreq && configForm.assetClass;
  };
    ...
    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 modal-animate">
      <div className="w-full md:max-w-5xl h-[85vh] bg-cp-black border border-cp-border flex flex-col shadow-2xl relative">
          ...
          <div className="flex justify-between items-center p-6 border-b border-cp-border bg-cp-dark shrink-0">
             ...
             <button onClick={onClose} className="text-gray-500 hover:text-cp-yellow transition-colors"><X size={24} /></button>
          </div>
          ...
          <div className="flex-1 overflow-hidden bg-cp-black relative flex flex-col min-h-0">
                    ...
                    <div className="w-full max-w-md space-y-12 text-center">
                        <div className="space-y-4">
                             <div className="w-24 h-24 mx-auto border border-cp-yellow/30 rounded-full flex items-center justify-center mb-6 bg-cp-dark shadow-[0_0_40px_rgba(197,160,89,0.1)] hover-card">
                                 <Brain size={40} className="text-cp-yellow opacity-80" strokeWidth={1} />
                             </div>
                             <h3 className="text-3xl font-serif font-bold text-white">初始化唯一 ID</h3>
                             <p className="text-cp-text-muted font-sans font-light">此名称将作为该智能体在 Matrix 网络中的唯一标识。</p>
                        </div>
            ...
            {step === 'preset' && (
                <div className="flex-1 flex flex-col p-8 animate-in fade-in slide-in-from-right-8 overflow-y-auto">
                    <div className="text-center mb-10">
                        <h3 className="text-2xl font-serif font-bold text-white mb-2">选择核心策略模组</h3>
                        <p className="text-cp-text-muted font-sans text-sm">为您的 Agent 选择一个行为原型。</p>
                    </div>
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        <preset.icon size={32} className={isSelected ? 'text-cp-yellow' : 'text-gray-500'} strokeWidth={1} />
                                        {isSelected && <div className="text-cp-yellow"><Check size={24} /></div>}
                                    </div>

                                    <div>
                                        <h4 className={`text-xl font-serif font-bold mb-2 ${isSelected ? 'text-white' : 'text-cp-text-muted'}`}>{preset.name}</h4>
                                        <p className="text-sm text-gray-500 font-sans leading-relaxed">{preset.desc}</p>
                                    </div>
                                </div>
                            );
                        })}
                    ...
                    <div className="mt-auto flex justify-center gap-6 pt-6 border-t border-cp-border">
                        <button onClick={() => setStep('naming')} className="px-8 py-3 btn-outline flex items-center gap-2"><ChevronLeft size={16} /> 返回</button>
                        <button 
            ...
            {/* STEP 3: CONFIGURE (New Step) */}
            {step === 'configure' && (
                <div className="flex-1 flex flex-col p-8 animate-in fade-in slide-in-from-right-8 overflow-y-auto">
                    <div className="max-w-4xl mx-auto w-full flex flex-col gap-8 pb-20">
                        <div className="text-center mb-4">
                            <h3 className="text-2xl font-serif font-bold text-white">系统参数配置</h3>
                            <p className="text-cp-text-muted text-sm">微调 {name} 的运行逻辑。</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-3">
                                <label className="text-xs font-bold text-cp-text-muted uppercase tracking-widest">风险偏好 <span className="text-cp-yellow">*</span></label>
                                <div className="grid grid-cols-3 gap-2">
                                    {['low', 'mid', 'high'].map(r => (
                                        <button 
                                            key={r} 
                                            onClick={() => setConfigForm({...configForm, riskLevel: r})}
                                            className={`py-3 border text-xs font-bold uppercase transition-colors ${configForm.riskLevel === r ? 'border-cp-yellow bg-cp-yellow text-black' : 'border-cp-border text-gray-500 hover:text-white'}`}
                                        >
                                            {r === 'low' ? '低' : r === 'mid' ? '中' : '高'}
                                        </button>
                                    ))}
                            ...
                            {/* Trading Freq */}
                            <div className="space-y-3">
                                <label className="text-xs font-bold text-cp-text-muted uppercase tracking-widest">交易频率 <span className="text-cp-yellow">*</span></label>
                                <select 
                                    value={configForm.tradingFreq}
                                    onChange={(e) => setConfigForm({...configForm, tradingFreq: e.target.value})}
                            ...
                            {/* Asset Class */}
                            <div className="space-y-3">
                                <label className="text-xs font-bold text-cp-text-muted uppercase tracking-widest">核心资产 <span className="text-cp-yellow">*</span></label>
                                <select 
                                    value={configForm.assetClass}
                                    onChange={(e) => setConfigForm({...configForm, assetClass: e.target.value})}
                                    className="w-full bg-cp-black border border-cp-border p-3 text-sm text-cp-text focus:border-cp-yellow outline-none appearance-none rounded-none"
                                >
                                    <option value="" disabled>选择资产类型</option>
                            ...
                            {/* Execution Style (Replaced Leverage) */}
                            <div className="space-y-3">
                                <label className="text-xs font-bold text-cp-text-muted uppercase tracking-widest">执行风格</label>
                                <select 
                                    value={configForm.executionStyle}
                                    onChange={(e) => setConfigForm({...configForm, executionStyle: e.target.value})}
                                    className="w-full bg-cp-black border border-cp-border p-3 text-sm text-cp-text focus:border-cp-yellow outline-none appearance-none rounded-none"
                                >
                    ...
                    <div className="mt-auto flex justify-center gap-6 pt-6 border-t border-cp-border bg-cp-black sticky bottom-0 z-10 p-4">
                        <button onClick={() => setStep('preset')} className="px-8 py-3 btn-outline flex items-center gap-2"><ChevronLeft size={16} /> 返回</button>
                        <button 
            ...
            {/* STEP 4: KNOWLEDGE (New Step) */}
            {step === 'knowledge' && (
                <div className="flex-1 flex flex-col p-8 animate-in fade-in slide-in-from-right-8">
                     <div className="text-center mb-8">
                        <h3 className="text-2xl font-serif font-bold text-white mb-2">知识库接入 (可选)</h3>
                        <p className="text-cp-text-muted font-sans text-sm">上传私有文档以增强 Agent 的分析能力。</p>
                    </div>
                    ...
                    <div className="max-w-3xl mx-auto w-full flex-1 flex flex-col gap-6">
                        ...
                        >
                            ...
                            <div className="w-16 h-16 rounded-full bg-cp-black border border-cp-border flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                                <Upload size={24} className="text-cp-text-muted group-hover:text-cp-yellow" />
                            </div>
                            <h4 className="text-lg font-bold text-cp-text mb-2">点击或拖拽上传文件</h4>
                            <p className="text-sm text-cp-text-muted">支持 PDF, TXT, MD, CSV 格式 (最大 20MB)</p>
                        </div>
                        ...
                        {uploadedFiles.length > 0 && (
                            <div className="border border-cp-border bg-cp-black p-4 max-h-[200px] overflow-y-auto custom-scrollbar">
                                {uploadedFiles.map((file, idx) => (
                                    <div key={idx} className="flex items-center justify-between p-3 border-b border-cp-border/50 last:border-0 hover:bg-cp-dim/30">
                                        <div className="flex items-center gap-3">
                                            <FileText size={16} className="text-cp-yellow" />
                                            <span className="text-sm text-cp-text">{file.name}</span>
                                            <span className="text-xs text-cp-text-muted">({file.size})</span>
                                        </div>
                                        <button onClick={() => removeFile(idx)} className="text-cp-text-muted hover:text-cp-red transition-colors">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                ))}
                    ...
                    <div className="mt-auto flex justify-center gap-6 pt-6 border-t border-cp-border">
                        <button onClick={() => setStep('configure')} className="px-8 py-3 btn-outline flex items-center gap-2"><ChevronLeft size={16} /> 返回</button>
                        <div className="flex gap-4">
                            <button onClick={() => setStep('simulation')} className="px-8 py-3 btn-outline text-cp-text-muted hover:text-white">跳过</button>
                            <button 
            ...
            {step === 'simulation' && (
                 <div className="flex-1 flex flex-col md:flex-row bg-cp-black animate-in fade-in slide-in-from-right-8">
                    ...
                    <div className="w-full md:w-1/3 border-r border-cp-border p-6 flex flex-col bg-cp-dark/5">
                        <div className="mb-6">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {(['1周', '1月', '3月', '1年'] as SimDuration[]).map(d => (
                                    <button 
                                        key={d} 
                        ...
                        <div className="flex-1 relative">
                             ...
                             <div className="absolute inset-0 border border-cp-border bg-black/50 p-4 font-mono text-xs text-cp-text-muted overflow-y-auto custom-scrollbar">
                                ...
                                {simStatus === 'running' && <div className="animate-pulse text-cp-yellow">> 处理中...</div>}
                             </div>
                        </div>
                    ...
                    <div className="flex-1 relative flex flex-col">
                         <div className="flex-1 relative bg-gradient-to-b from-cp-black to-cp-dark/20" ref={chartRef}>
                         ...
                         <div className="h-24 border-t border-cp-border bg-cp-black flex items-center justify-around px-8">
                             {[
                                 { label: '总收益', val: simResult ? `+${simResult.duration === '1周' ? '3.2' : '15.4'}%` : '--', color: 'text-cp-yellow' },
                                 { label: '最大回撤', val: simResult ? '-2.1%' : '--', color: 'text-white' },
                                 { label: '胜率', val: simResult ? '62%' : '--', color: 'text-white' },
                                 { label: '夏普比率', val: simResult ? '1.85' : '--', color: 'text-white' },
                             ].map((stat, i) => (
                                 <div key={i} className={`flex flex-col items-center gap-1 ${!simResult ? 'opacity-50 blur-[2px]' : ''} transition-all`}>
                                     <span className="text-[10px] text-cp-text-muted uppercase tracking-widest">{stat.label}</span>
                                     <span className={`text-xl font-bold font-mono ${stat.color}`}>{stat.val}</span>
                                 </div>
                             ))}
                         </div>
                    </div>
            ...
            {/* Footer Buttons for Simulation Step */}
            {step === 'simulation' && (
                <div className="p-4 border-t border-cp-border bg-cp-dark flex justify-between items-center shrink-0">
                    <button onClick={() => setStep('knowledge')} className="px-6 py-2 text-cp-text-muted hover:text-white flex items-center gap-2 text-sm font-bold">
