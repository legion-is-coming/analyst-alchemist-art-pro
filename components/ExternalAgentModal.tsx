...
import { X, Hexagon, Activity, Crosshair, Search, Rewind, PenTool, FileText, CheckCircle } from 'lucide-react';
...
const ExternalAgentModal: React.FC<ExternalAgentModalProps> = ({ agentName, onClose }) => {
  const [activeTab, setActiveTab] = useState<AgentCapability>(AgentCapability.AUTO_TRADING);
      ...
      <div className="w-full max-w-4xl h-[80vh] bg-cp-black border border-cp-border flex flex-col shadow-2xl relative">
        ...
        <div className="flex items-center justify-between p-6 border-b border-cp-border bg-cp-dark shrink-0">
             <div className="flex items-center gap-6">
                ...
                <div>
                    <h2 className="text-3xl font-bold font-serif text-white tracking-wide uppercase">{agentName}</h2>
                    <div className="flex items-center gap-4 text-xs font-mono mt-2 text-cp-text-muted">
                        <span>RANK: #3</span>
                        <span className="text-gray-600">|</span>
                        <span>STATUS: <span className="text-cp-yellow">ONLINE</span></span>
                    </div>
                </div>
             </div>
        ...
        <div className="flex flex-1 overflow-hidden">
                    ...
                    return (
                        <button
                            ...
                            className={`px-6 py-4 text-left flex items-center gap-3 transition-all relative hover-card m-2 border-transparent
                                ${isActive ? 'text-cp-yellow border border-cp-yellow/30 bg-cp-dark/30' : 'text-gray-500 hover:text-gray-300'}
                            `}
                        >
                            <span className="font-bold font-sans text-xs uppercase tracking-widest">{details.label}</span>
                        </button>
                    );
                })}
            </div>
            ...
            <div className="flex-1 p-8 bg-cp-black overflow-y-auto custom-scrollbar">
                <div className="mb-6 pb-4 border-b border-cp-border">
                     <h3 className="text-xl font-serif font-bold text-white mb-2">{AGENT_CAPABILITY_DETAILS[activeTab].label}</h3>
                     <p className="text-sm text-gray-500 font-sans">{AGENT_CAPABILITY_DETAILS[activeTab].desc}</p>
                </div>
                ...
                <div className="font-mono text-sm text-cp-text-muted leading-loose">
                    ...
                    <div className="p-4 border border-cp-border bg-cp-dark/20 mb-4 hover-card">
                        ...
                        <div className="text-white font-bold">BUY 600519.SH @ 1680.00</div>
                    </div>
                    <p className="hover-card p-4 border border-transparent">
                        Analysis Engine active. Monitoring volatility index for entry points. <br/>
                        Detected divergence in sector momentum. <br/>
                        <span className="text-cp-yellow">> Executing limit order strategy...</span>
                    </p>
                </div>
            </div>
        </div>
...
export default ExternalAgentModal;