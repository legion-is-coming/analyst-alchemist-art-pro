...
import { Hexagon, Fingerprint, Power, UserPlus, Lock, Mail, Globe, User, X } from 'lucide-react';
        ...
        <div className="w-full md:w-5/12 bg-cp-dark p-10 flex flex-col justify-between relative overflow-hidden border-b md:border-b-0 md:border-r border-cp-border hover-card m-1 border-transparent">
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-3 text-cp-yellow mb-8">
                    ...
                    <span className="font-bold font-serif text-xl tracking-wider">MATRIX</span>
                </div>
            ...
            <div className="relative z-10 hidden md:block">
                <div className="py-4 border-t border-cp-border">
                    <div className="text-xs text-cp-yellow font-bold uppercase tracking-widest mb-2">{t('login.season_name')}</div>
        ...
        <div className="w-full md:w-7/12 p-10 md:p-14 bg-cp-black flex flex-col justify-center">
            ...
            <div className="flex gap-8 mb-10 border-b border-cp-border">
                <button 
                    ...
                    className={`pb-4 text-sm font-bold uppercase tracking-widest transition-colors tab-item ${mode === 'login' ? 'active' : ''}`}
                >
                    {t('login.tab_login')}
                </button>
                <button 
                    onClick={() => setMode('register')}
                    className={`pb-4 text-sm font-bold uppercase tracking-widest transition-colors tab-item ${mode === 'register' ? 'active' : ''}`}
                >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-6">
                    <div className="relative group hover-card p-2 border border-transparent">
                        <label className="text-[10px] uppercase font-bold text-gray-500 tracking-widest mb-1 block group-focus-within:text-cp-yellow transition-colors">Username</label>
                        <input 
                            type="text" 
                            value={username}
                    ...
                    {mode === 'register' && (
                        <div className="relative group animate-in slide-in-from-top-2 hover-card p-2 border border-transparent">
                             <label className="text-[10px] uppercase font-bold text-gray-500 tracking-widest mb-1 block group-focus-within:text-cp-yellow transition-colors">Email</label>
                            <input 
                                type="email" 
                                value={email}
                    ...
                    <div className="relative group hover-card p-2 border border-transparent">
                         <label className="text-[10px] uppercase font-bold text-gray-500 tracking-widest mb-1 block group-focus-within:text-cp-yellow transition-colors">Password</label>
                        <input 
                            type="password" 
                            value={password}
                ...
                >
                    {isConnecting ? (
                        <span className="animate-spin"><Power size={18} /></span>
                    ) : (
                        <Fingerprint size={18} />
                    )}
                    {isConnecting ? t('login.connecting') : mode === 'login' ? t('login.connect') : t('login.mint')}
                </button>
            </form>
