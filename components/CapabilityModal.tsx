...
import { X, Cpu, Play, RotateCcw, Save, Clock, ChevronRight } from 'lucide-react';
import { AgentCapability, AGENT_CAPABILITY_DETAILS, AppNotification } from '../types';
...
const CapabilityModal: React.FC<CapabilityModalProps> = ({ capability, onClose }) => {
  ...
  const handleExecute = () => {
    setIsLoading(true);
    setTimeout(() => {
        setOutput("Analysis Complete. Signal Strong Buy.");
        setIsLoading(false);
    }, 1500);
  };
      ...
      <div className="w-full md:max-w-6xl h-[85vh] bg-cp-black border border-cp-border flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between p-6 bg-cp-dark border-b border-cp-border shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold font-serif tracking-wide uppercase text-white">{details.label} Module</h2>
              <p className="text-xs text-cp-text-muted font-sans mt-0.5">Role: {details.role}</p>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-cp-black">
          ...
          <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-cp-border p-6 flex flex-col bg-cp-dark/10 hover-card m-2 border-transparent">
            <label className="text-cp-text-muted text-xs font-bold uppercase tracking-widest mb-4 block">Input Parameters</label>
            <textarea 
              value={input}
            ...
            >
              {isLoading ? <RotateCcw className="animate-spin" size={18} /> : <Play size={18} />}
              EXECUTE TASK
            </button>
          </div>
          ...
          <div className="flex-1 p-8 flex flex-col bg-cp-black relative">
            <label className="text-cp-text-muted text-xs font-bold uppercase tracking-widest mb-4 block">Execution Output</label>
            <div className="flex-1 border border-cp-border bg-cp-dark/20 p-6 font-mono text-sm text-cp-text overflow-y-auto custom-scrollbar leading-relaxed hover-card">
              {output ? output : <span className="text-gray-600 italic">Waiting for input...</span>}
            </div>
