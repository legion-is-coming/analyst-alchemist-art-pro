...
import { Trophy, Brain, Zap, Shield, Hexagon, Activity, User, TrendingUp } from 'lucide-react';
...
const RankingList: React.FC<RankingListProps> = ({ data, onSelectAgent, onInspectAgent }) => {
  const { t } = useLanguage();
  const listRef = useRef<HTMLDivElement>(null);
  const rowRefs = useRef<Map<string | number, HTMLDivElement>>(new Map());
  const isInitialized = useRef(false);

  // GSAP Animation for Reordering
  useLayoutEffect(() => {
    ...
    scrollableItems.forEach((agent, index) => {
      ...
      if (el) {
        const targetY = index * 60; // Increased row height for larger text

        if (!isInitialized.current) {
            // Instant set for first render - No animation
            gsap.set(el, { y: targetY, opacity: 1, x: 0 });
            el.setAttribute('data-revealed', 'true');
        } else {
            // Simple position update for subsequent renders
            gsap.to(el, {
              y: targetY,
              duration: 0.5,
              ease: "power2.out",
            ...
            // Staggered reveal if not already visible (for new items)
            if (!el.hasAttribute('data-revealed')) {
                 gsap.fromTo(el, { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.4, delay: index * 0.05 });
                 el.setAttribute('data-revealed', 'true');
            }
        }
      }
  ...
  const getClassIcon = (cls: string) => {
      switch(cls) {
          case 'Quant': return <Brain size={16} className="text-cp-cyan"/>;
          case 'Scalper': return <Zap size={16} className="text-cp-yellow"/>;
          case 'Whale': return <Shield size={16} className="text-cp-red"/>;
          default: return <Hexagon size={16} className="text-cp-text-muted"/>;
      }
  };

  const ROW_HEIGHT = 60; // Larger row height
  ...
  const renderRow = (agent: RankingItem, isPinned: boolean = false) => (
      <div 
        key={agent.id}
        ref={!isPinned ? (el) => { if (el) rowRefs.current.set(agent.id, el); } : undefined}
        style={!isPinned ? {
            position: 'absolute',
            top: 0, 
            left: 0,
        ...
        className={`group flex items-center px-6 transition-all duration-300 cursor-pointer hover:bg-cp-dim overflow-hidden
            ${isPinned ? 'bg-cp-yellow/5 border-b border-cp-yellow/30' : 'bg-cp-black'}`}
      ...
      >
        ...
        <div className="w-16 shrink-0 font-serif font-bold text-lg text-cp-text-muted group-hover:text-cp-text flex items-center gap-2 group-hover:translate-x-2 transition-transform">
            ...
            <span className={agent.rank <= 3 ? 'text-cp-yellow' : ''}>{agent.rank < 10 ? `0${agent.rank}` : agent.rank}</span>
        </div>

        {/* Name */}
        <div className="flex-1 min-w-0 flex items-center gap-4 group-hover:translate-x-2 transition-transform">
            <div className={`w-8 h-8 flex items-center justify-center shrink-0 border border-cp-border bg-cp-dark ${isPinned ? 'border-cp-yellow/30' : ''}`}>
                {getClassIcon(agent.class)}
            </div>
            <div className="flex flex-col justify-center">
                 <span className={`font-serif font-bold text-base truncate ${isPinned ? 'text-cp-yellow' : 'text-cp-text'} group-hover:text-white`}>
                    {agent.name}
                 </span>
                 <span className="text-[10px] uppercase tracking-wider text-cp-text-muted">{agent.class} MODEL</span>
            </div>
        </div>

        {/* Profit */}
        <div className={`w-28 text-right font-mono font-bold text-base ${agent.rawProfit > 0 ? 'text-cp-yellow' : agent.rawProfit < 0 ? 'text-cp-red' : 'text-cp-text-muted'} group-hover:scale-105 transition-transform`}>
            {agent.profit}
        </div>
        
        {/* Action Button */}
        {!isPinned && <div className="w-12 flex justify-end opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0">
            <button 
                onClick={(e) => { e.stopPropagation(); onInspectAgent(agent.name); }}
  ...
  return (
    <div className="w-full h-full flex flex-col min-h-0 bg-cp-black">
      ...
      {/* Header Row - Clean, No Bottom Border */}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 min-h-0">
        <div 
            ref={listRef} 
            style={{ height: `${otherAgents.length * ROW_HEIGHT}px`, position: 'relative' }}
        >
