...
const MatrixRain: React.FC<MatrixRainProps> = ({ theme = 'dark' }) => {
  ...
  useEffect(() => {
    ...
    const particles: any[] = [];
    const particleCount = 30; // Fewer, larger particles for abstract look
    ...
    class Particle {
      ...
      constructor() {
        ...
        this.size = Math.random() * 300 + 150; // Very large
      ...
      draw() {
        if (!ctx) return;
        ctx.beginPath();
        // Soft radial gradient for "Ink" look
    ...
    const draw = () => {
      ...
      // Clear with very slight fade for trail effect? No, simple repaint for cleanliness in this style
