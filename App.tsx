...
import { Hexagon, Activity, User as UserIcon, LogOut, LogIn, Trophy, Sun, Moon, Users, Bell, Languages, Palette } from 'lucide-react';
...
const INITIAL_BOTS: RankingItem[] = [
    { id: 101, rank: 1, name: "Alpha_Seeker", class: 'Quant', profit: "+0.0%", rawProfit: 100, status: '在线', badges: ['legend', 'whale'] },
    { id: 102, rank: 2, name: "Golden_Ratio", class: 'Scalper', profit: "+0.0%", rawProfit: 100, status: '在线', badges: ['quant'] },
    { id: 103, rank: 3, name: "Value_Discovery", class: 'Quant', profit: "+0.0%", rawProfit: 100, status: '在线' },
    { id: 104, rank: 4, name: "Momentum_Flow", class: 'Scalper', profit: "+0.0%", rawProfit: 100, status: '训练中', badges: ['risk'] },
    { id: 105, rank: 5, name: "Reform_Strategy", class: 'Corpo', profit: "+0.0%", rawProfit: 100, status: '离线' },
    { id: 106, rank: 6, name: "Growth_Hunter", class: 'NetRunner', profit: "+0.0%", rawProfit: 100, status: '在线' },
    { id: 107, rank: 7, name: "Arbitrage_Bot", class: 'Quant', profit: "+0.0%", rawProfit: 100, status: '在线' },
    { id: 108, rank: 8, name: "Dividend_Yield", class: 'Whale', profit: "+0.0%", rawProfit: 100, status: '在线' },
    { id: 109, rank: 9, name: "Limit_Up", class: 'Scalper', profit: "+0.0%", rawProfit: 100, status: '训练中' },
    { id: 110, rank: 10, name: "Micro_Cap", class: 'NetRunner', profit: "+0.0%", rawProfit: 100, status: '在线' },
];
...
const App: React.FC = () => {
  ...
  const handleSavePrompt = (prompt: string) => {
    if (editingCapability) {
        ...
        addNotification("Configuration Saved", `[${AGENT_CAPABILITY_DETAILS[editingCapability].label}] prompt updated.`, 'success');
    }
  };
  ...
  const handleLogin = (username: string, email?: string, faction?: string) => {
    ...
    addNotification("Authenticated", `Welcome, ${username}.`, 'success');
  ...
  const handleLogout = () => {
      ...
      addNotification("Disconnected", "User logged out securely.", 'info');
  };
  ...
  const handleCreateAgent = (name: string, prompt: string, archetype: string, stats: AgentStats, modules: AgentModule[]) => {
    ...
    addNotification("Agent Deployed", `${name} is ready. Join season to compete.`, 'success');
  };
  ...
  const handleDeleteAgent = () => {
      setConfirmModal({
          ...
          message: 'Are you sure you want to destroy this Agent instance? This action is irreversible.',
          action: () => {
              clearAgentData(); 
              setAgentName(null);
              ...
              addNotification("Destruction Complete", "Agent instance removed.", 'warning');
          }
      });
  };
  ...
  const handleConfirmJoinCompetition = () => {
      ...
      addNotification("Competition Joined", "Agent entered Season 4 pool.", 'success');
  };
  ...
  const handleToggleJoin = () => {
      ...
      const title = "Withdraw from Season";
      const message = "Are you sure? You will stop earning season points and your rank will freeze.";
      ...
      setConfirmModal({
          ...
          action: () => {
              ...
              competitorsRef.current = competitorsRef.current.filter(c => !c.isUser);
              setRankingList(prev => prev.filter(c => !c.isUser));
              addNotification("Withdrawn", "Exited competition. Records archived.", 'info');
          }
      });
  };
      ...
      {showLanding ? (
        ...
        <header className="flex justify-between items-center z-50 px-6 h-[60px] shrink-0 border-b border-cp-border bg-cp-black">
          ...
          <div className="flex items-center gap-0 h-full border-l border-cp-border">
             <div className="hidden lg:flex items-center gap-4 px-6 text-xs font-bold font-sans text-cp-text-muted tracking-widest border-r border-cp-border h-full">
             ...
             <div className="flex items-center h-full">
                 <button 
                    onClick={toggleTheme}
                    className="w-12 h-full hover:bg-cp-dim text-cp-text-muted hover:text-cp-yellow transition-colors flex items-center justify-center border-r border-cp-border"
                 >
                    {theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}
                 </button>
                 <button 
                    onClick={() => setLanguage(language === 'zh' ? 'en' : 'zh')}
                    className="w-12 h-full hover:bg-cp-dim text-cp-text-muted hover:text-cp-yellow transition-colors flex items-center justify-center border-r border-cp-border font-bold text-xs font-serif"
                 >
                    {language.toUpperCase()}
                 </button>
                 <button 
                    onClick={() => setIsNotifHistoryOpen(true)}
                    className="w-12 h-full hover:bg-cp-dim text-cp-text-muted hover:text-cp-yellow transition-colors flex items-center justify-center border-r border-cp-border relative"
                 >
                    <Bell size={18} />
             ...
             {currentUser ? (
                     ...
                     {isUserMenuOpen && (
                        <div className="absolute top-full right-0 w-64 bg-cp-black border border-cp-border shadow-2xl z-50 animate-in fade-in slide-in-from-top-2">
                            ...
                            <button 
                                ...
                                className="w-full flex items-center gap-2 p-4 text-cp-red hover:bg-cp-dim text-xs font-bold font-sans uppercase tracking-widest transition-colors"
                            >
                 ...
                 <button 
                    ...
                    className="px-8 h-full bg-cp-yellow text-black hover:bg-white transition-colors font-bold font-sans text-xs uppercase flex items-center gap-2 tracking-widest"
                 >
                    <LogIn size={14} />
        ...
        <div className="md:hidden flex h-[50px] border-b border-cp-border bg-cp-black shrink-0">
            <button 
                ...
                className={`flex-1 font-bold uppercase flex items-center justify-center gap-2 transition-colors border-r border-cp-border text-xs tracking-widest ${mobileView === 'MARKET' ? 'bg-cp-yellow text-black' : 'text-cp-text-muted'}`}
            >
               <Activity size={14} /> {t('nav.market')}
            </button>
            <button 
                onClick={() => { setMobileView('SIDEBAR'); setActiveSideTab('MY_AGENT'); }} 
                className={`flex-1 font-bold uppercase flex items-center justify-center gap-2 transition-colors border-r border-cp-border text-xs tracking-widest ${(mobileView === 'SIDEBAR' && activeSideTab === 'MY_AGENT') ? 'bg-cp-yellow text-black' : 'text-cp-text-muted'}`}
            >
               <Users size={14} /> {t('nav.team')}
            </button>
            <button 
                onClick={() => { setMobileView('SIDEBAR'); setActiveSideTab('RANKING'); }} 
                className={`flex-1 font-bold uppercase flex items-center justify-center gap-2 transition-colors text-xs tracking-widest ${(mobileView === 'SIDEBAR' && activeSideTab === 'RANKING') ? 'bg-cp-cyan text-black' : 'text-cp-text-muted'}`}
            >
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0 bg-cp-black">
          ...
          <section className={`${mobileView === 'MARKET' ? 'flex flex-1' : 'hidden'} md:flex w-full md:w-[70%] flex-col min-h-0 border-r border-cp-border`}>
            {/* Chart Area */}
          ...
          <aside className={`${mobileView === 'SIDEBAR' ? 'flex flex-1' : 'hidden'} md:flex w-full md:w-[30%] flex-col bg-cp-black`}>
            ...
            <div className="hidden md:flex items-center shrink-0 h-[50px] border-b border-cp-border bg-cp-dark">
                <button 
                    ...
                    className={`flex-1 h-full flex items-center justify-center gap-2 font-bold font-sans uppercase text-xs tracking-widest transition-colors border-r border-cp-border hover:bg-cp-dim ${activeSideTab === 'MY_AGENT' ? 'text-cp-yellow bg-cp-black' : 'text-cp-text-muted'}`}
                >
                    <Users size={14} /> {t('nav.my_team')}
                </button>
                <button 
                    onClick={() => setActiveSideTab('RANKING')}
                    className={`flex-1 h-full flex items-center justify-center gap-2 font-bold font-sans uppercase text-xs tracking-widest transition-colors hover:bg-cp-dim ${activeSideTab === 'RANKING' ? 'text-cp-cyan bg-cp-black' : 'text-cp-text-muted'}`}
                >
